...
def run(action, params):
    ...
    try:
        if action == "products.list":
            ...
            if payment_status:
                c.execute("UPDATE orders SET status=?, payment_status=? WHERE id=?",
                          (params["status"], payment_status, params["id"]))
            else:
        ...
        elif action == "newsletter.list":
            c.execute("SELECT email, subscribed_at FROM newsletter ORDER BY subscribed_at DESC")
            ...
            c.execute("SELECT COALESCE(SUM(total),0) as rev FROM orders WHERE status='paid' OR payment_status='paid'")
